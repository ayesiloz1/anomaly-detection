...
class Decoder(nn.Module):
    def __init__(self, img_shape, latent_dim):
        ...
        self.model = nn.Sequential(
            ...
            # State (32x128x128)
            nn.ConvTranspose2d(in_channels=32, out_channels=3, kernel_size=4, stride=2, padding=1),  # Set to 1 channel (grayscale)
            nn.Tanh()
        )

    def forward(self, input):
        img = self.model(input)
        return img.view(img.shape[0], *self.img_shape)  # Should return (batch_size, channels, height, width)
